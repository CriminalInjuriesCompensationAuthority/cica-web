...
Modal.prototype.handleBeforeOpen = function (e) {
  ...
  if (typeof this.options.onBeforeOpen === 'function') {
    var response = this.options.onBeforeOpen();
    if (response === false) {
      shouldOpen = false;
    }
  }
...
Modal.prototype.handleOpen = function (e) {
  ...
  var event = new (custom_event__WEBPACK_IMPORTED_MODULE_0___default())('MODAL_OPEN');
...
Modal.prototype.handleClose = function (e) {
  ...
  var event = new (custom_event__WEBPACK_IMPORTED_MODULE_0___default())('MODAL_CLOSE');
  this.module.dispatchEvent(event);
};
...
Modal.prototype.handleContent = function (updateContent) {
  var titleElement = this.dialogBox.querySelector('.govuk-modal__heading');
  var bodyElement = this.dialogBox.querySelector('.govuk-modal__content');
...
class Unit {
  ...
  from() {
    ...
    return function () {
      ...
      return units.reduce((sum, unit, i) => sum + ns[i] * unit.magnitude, 0) / _this.magnitude;
    };
  }
}
  ...
  function init(elements) {
    ...
    if (selectElements.length) {
      var _loop = function () {
        ...
        var hintElement = window.document.querySelector('.govuk-hint');

        // Define the default assistive hint
        var assistiveHint = 'When autocomplete results are available use up and down arrows to review and enter to select. Touch device users, explore by touch or with swipe gestures.';

        // Append the innerText of hintElement to assistiveHint if hintElement exists
        if (hintElement && hintElement.innerText.trim() !== '') {
          assistiveHint = "".concat(hintElement.innerText, " ").concat(assistiveHint);
        }
...
function createCookieBanner(window, cookiePreference) {
  ...
  if (formCookiePreference) {
    var preferencesElements = formCookiePreference.querySelectorAll('[data-cookie-preference]');
    ...
    formCookiePreference.addEventListener('submit', e => {
      var preferencesElementsSelected = formCookiePreference.querySelectorAll('[data-cookie-preference]:checked');
...
function createCicaGa(window) {
  ...
  function init() {
    var trackableElements = window.document.querySelectorAll('[data-module], .ga-event, .govuk-input, .govuk-textarea');
...
function createTimeoutModal(window) {
  ...
  function TimeoutModal(options) {
    var element = window.document.querySelector(options.element);
    var closeElement = element.querySelector(options.closeElement); // closeElement must be a child element of `element`.
    var triggerElement = window.document.querySelector(options.triggerElement); // closeElement must be a child element of `element`.
    var openInTimeout;
    var closeInTimeout;
    var config = {
      element,
    ...
    var modal = new _components_cica_modal_modal__WEBPACK_IMPORTED_MODULE_0__["default"](element).init({
      triggerElement,
      closeElement,
    ...
    function setUp() {
      ...
      if (config.timer) {
        ...
        var timerDuration = setUpOptions.duration || duration;
        var timerStartTime = setUpOptions.startTime || startTime;
        var timer = new _utils_AccurateInterval__WEBPACK_IMPORTED_MODULE_1__["default"]({
          onTick,
          onEnd,
          interval,
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function AccurateInterval(_ref) {
  var {
    onTick,
    onEnd,
    interval,
...
function msToMinutesAndSeconds(duration) {
  var result = time_convert__WEBPACK_IMPORTED_MODULE_0__.ms.to(time_convert__WEBPACK_IMPORTED_MODULE_0__.m, time_convert__WEBPACK_IMPORTED_MODULE_0__.s)(duration);
  ...
  return "".concat(minutesText).concat(conjunctionText).concat(secondsText) || "0 seconds";
}
...
function createPostcodeLookup(window) {
  ...
  // eslint-disable-next-line no-unused-vars
  var tmpAddressSearchResultsJson = {};
  var apiNoAddressesFoundErrorMessage;
  var apiResponseNotOkErrorMessage;
  var INVALID_POSTCODE_ERROR = 'Enter a valid postcode';
  var emptySearchInputErrorMessage;
  var searchResultsDiv;
  var selectElementResults;
  var addressSearchResultsOptions;
  var addressSearchInput;

  // Matches a single letter (e.g 'A'), number range (e.g. 110-114), delimiter (e.g. 1/2)
  // Does not match 'Flat A'
  var flatIdentifierRegex = /((^\d\S+([a-zA-Z]|\d)$|^[a-zA-Z]$))/;
  ...
  function clearAddressForm() {
    var elements = window.document.querySelectorAll("form input[type='text']");
    elements.forEach(el => {
      var addressInput = el;
      addressInput.value = '';
    });
  ...
  function mapSelectionToAddressFormInputs() {
    ...
    var dataset = 'DPA';
    var result = tmpAddressSearchResultsJson[selectedValue][dataset];
    clearAddressForm();
    ...
    var dpaPostcode = result.POSTCODE;

    // Add a "PO BOX " prefix to the PO Box Number integer.
    if (dpaPOBoxNumber !== '') {
      dpaPOBoxNumber = "PO BOX ".concat(dpaPOBoxNumber);
    }

    // Remove empty values from the array
    var thoroughfareLocality = [dpaDependentThoroughfareName, dpaThoroughfareName, dpaDependentLocality].filter(item => item);
    var premises = [];
    if (dpaSubBuildingName !== '' && dpaBuildingName !== '' && flatIdentifierRegex.test(dpaSubBuildingName)) {
      dpaBuildingName = "".concat(dpaSubBuildingName, " ").concat(dpaBuildingName);
    } else {
      premises.push(dpaSubBuildingName);
    }
    var premisesThoroughfareLocality = '';
    if (flatIdentifierRegex.test(dpaBuildingName)) {
      // add to thouroughfare
      premisesThoroughfareLocality = "".concat(dpaBuildingName, " ").concat(thoroughfareLocality[0]);
      thoroughfareLocality.shift();
    } else {
      premises.push(dpaBuildingName);
    }
    if (dpaBuildingNumber !== '') {
      premisesThoroughfareLocality = "".concat(dpaBuildingNumber, " ").concat(thoroughfareLocality[0]);
      thoroughfareLocality.shift();
    }
    var addressLines = [];

    // Does the page contain an organisation name?
    var organisationName = window.document.querySelector('input[id *= "organisation-name"]');
    if (organisationName) {
    ...
    var [selectionLine1, selectionLine2, selectionLine3] = addressLines;
    var addressLine1 = window.document.querySelector('input[id *= "street"]');
    addressLine1.value = selectionLine1;
    var addressLine2 = window.document.querySelector('input[id *= "street-2"]');
    addressLine2.value = selectionLine2 || '';
    var addressLine3 = window.document.querySelector('input[id *= "street-3"]');
    addressLine3.value = selectionLine3 || '';
    var addressTown = window.document.querySelector('input[id *= "town-or-city"]');
    addressTown.value = dpaPostTown;
    var addressPostcode = window.document.querySelector('input[id *= "postcode"]');
  ...
  function addSearchResultsToSelectElement(data) {
    ...
    option.text = data.header.totalresults === 1 ? '1 address found' : "".concat(data.header.totalresults, " addresses found");
  ...
  var addressSearchDiv;
  function displayFieldErrors(message) {
    var errorInnerSpan = window.document.createElement('span');
    errorInnerSpan.setAttribute('class', 'govuk-visually-hidden');
    var errorInnerSpanText = window.document.createTextNode('Error:');
    errorInnerSpan.appendChild(errorInnerSpanText);
    var errorOuterSpan = window.document.createElement('span');
    errorOuterSpan.setAttribute('id', 'address-search-input-error');
    errorOuterSpan.setAttribute('class', 'govuk-error-message');
    var errorOuterSpanText = window.document.createTextNode(message);
  ...
  function handleErrorSummaryListElement(message) {
    ...
    var errorAnchor = window.document.createElement('a');
    var link = window.document.createTextNode(message);
    errorAnchor.appendChild(link);
    errorAnchor.href = '#address-search-input';
    /* eslint-disable func-names */
    errorAnchor.onclick = function () {
      var assocLabel = window.document.querySelector("label[for='".concat(addressSearchInput.id, "']"));
  ...
  function removeErrorMessages() {
    var pageTitle = window.document.title.replace('Error: ', '');
    // eslint-disable-next-line no-param-reassign
    window.document.title = pageTitle;
    var errorSummary = window.document.querySelector('.govuk-error-summary');
    if (errorSummary) {
      errorSummary.remove();
    } else {
      return;
    }
    var errorSpans = window.document.querySelectorAll('.govuk-error-message');
    errorSpans.forEach(errorSpan => {
      errorSpan.remove();
    });
    var errorInputs = window.document.querySelectorAll('.govuk-input--error');
    errorInputs.forEach(errorInput => {
      var inputClass = errorInput.getAttribute('class');
      errorInput.setAttribute('class', inputClass.replace('govuk-input--error', ''));
      errorInput.removeAttribute('aria-describedby');
    });
    var errorDivs = window.document.querySelectorAll('.govuk-form-group--error');
  ...
  function removeAddressSearchElements() {
    ...
    var hintTextDiv = window.document.getElementById('fill-out-the-fields-manually-hint');
    hintTextDiv.remove();
    var addressSearchButton = window.document.getElementById('search-button');
    addressSearchButton.remove();
    var addressSearchResults = window.document.getElementById('address-search-results');
    addressSearchResults.remove();
  }
  function sendAnalyticsEvent(message) {
    var action = message === apiResponseNotOkErrorMessage ? 'systemError' : 'validationError';
    var gtagOptions = {
      type: 'event',
      action,
  ...
  function displayErrors(message) {
    ...
    clearAddressResultsDropdown();
    var pageErrorTitle = "Error: ".concat(window.document.title);
    // eslint-disable-next-line no-param-reassign
  ...
  function addressSearch() {
    return _addressSearch.apply(this, arguments);
  }
  function _addressSearch() {
    _addressSearch = _asyncToGenerator(function* () {
      removeErrorMessages();
      clearAddressForm();
      var addressSearchInputValue = addressSearchInput.value;
      ...
      var response = yield fetch("/address-finder/postcode?postcode=".concat(addressSearchInputValue));
      if (!response.ok) {
  ...
  function createPostcodeSearchElements() {
    var addressSearchLabelContent = window.document.createTextNode('Search for a UK address using a postcode (optional)');
    var addressSearchLabel = window.document.createElement('label');
  ...
  function createFindAddressButton() {
    var searchButton = window.document.createElement('button');
  ...
  function createSearchResultsElements() {
    ...
    var addressSearchResultsLabel = window.document.createElement('label');
    ...
    var searchResults = window.document.createElement('select');
  ...
  function setContextualisationMessages() {
    var h1TextContent = window.document.querySelector('h1').textContent;
    var pageContext = 'the';
    if (h1TextContent.includes('their') || h1TextContent.includes('your')) {
      pageContext = h1TextContent.includes('their') ? 'their' : 'your';
    }
    apiNoAddressesFoundErrorMessage = "We could not find any addresses for that postcode. Check ".concat(pageContext, " postcode is correct, or enter ").concat(pageContext, " address manually.");
    apiResponseNotOkErrorMessage = "The system is experiencing an issue. Enter ".concat(pageContext, " address manually.");
    emptySearchInputErrorMessage = "Enter ".concat(pageContext, " postcode");
  }
  function init() {
